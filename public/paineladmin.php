...
if (isset($_POST['adicionar_tarefa'])) {
    ...
    $buscarMateria = $conexao->prepare(" SELECT nm_materia FROM materias  WHERE cd_materia= ? ");
    $buscarMateria->bind_param("i", $materia);
    $buscarMateria->execute();

    $resultMateria = $buscarMateria->get_result();

    $dadosMateria = $resultMateria->fetch_assoc();

    $nomeMateria = $dadosMateria['nm_materia'];

    $titulo = "Nova tarefa: " . $nomeMateria;
    $mensagem = "Uma nova atividade foi adicionada.";
    $inserirnotificacao = $conexao->prepare(" INSERT INTO `notificacoes` ( `id_usuario`, `titulo`, `mensagem`, `tipo`, `lida`, `link`, `data_criacao`) VALUES ( '0', ?, ?, 'atividade', '0', '0', current_timestamp())");
    $inserirnotificacao->bind_param("ss", $titulo, $mensagem, );
...
if (isset($_POST['atualizar_tarefa'])) {
    ...
    $titulo = "Tarefa Atualizada: " . $nomeMateria;
    $mensagem = "Uma  atividade foi Atualizada.";
    $inserirnotificacao = $conexao->prepare(" INSERT INTO `notificacoes` ( `id_usuario`, `titulo`, `mensagem`, `tipo`, `lida`, `link`, `data_criacao`) VALUES ( '0', ?, ?, 'atividade', '0', '0', current_timestamp())");
    $inserirnotificacao->bind_param("ss", $titulo, $mensagem, );
    $inserirnotificacao->execute();
    header("Location: " . $_SERVER['PHP_SELF']);
...
if (isset($_POST['confirmar_exclusao'])) {
    ...
    $buscarMateria = $conexao->prepare(" SELECT nm_materia FROM materias  WHERE cd_materia= ? ");
    $buscarMateria->bind_param("i", $cd_atividade);
    $buscarMateria->execute();
    ...
    $titulo = "Tarefa Excluida: " . $nomeMateria;
    $mensagem = "Atividade foi Excluida.";
    $inserirnotificacao = $conexao->prepare(" INSERT INTO `notificacoes` ( `id_usuario`, `titulo`, `mensagem`, `tipo`, `lida`, `link`, `data_criacao`) VALUES ( '0', ?, ?, 'atividade', '0', '0', current_timestamp())");
    $inserirnotificacao->bind_param("ss", $titulo, $mensagem, );
    $inserirnotificacao->execute();
